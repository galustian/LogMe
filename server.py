...
def get_days_dict(from_date, to_date):
    ...
    for log_y in logs:
        for log_d in logs[log_y]:
            log_date = log_d.split("-")
            log_day = int(log_date[0])
            log_month = int(log_date[1])

            if from_month < log_month < to_month:
                days[log_d] = logs[log_y][log_d]
...
def create_graph(days_avg):
    ...
    for i, app_name in enumerate(app_names_list):
        ...
        plt.plot_date(date_obj_list, hours_list, color=app_colors_dict[app_name], label=app_name, markeredgewidth=2, linestyle='-', lw=3)
...
def create_bar(days_avg):
    ...
    for i, app_name in enumerate(app_names_list):
        ...
        plt.bar(date_obj_list, hours_list, color=app_colors_dict[app_name], label=app_name, width=0.8)
        ax = plt.gca()
        ax.xaxis_date()
...
@app.route('/<date_range>/<max_points_in_chart>', methods=['GET', 'POST'])
def homepage(date_range, max_points_in_chart):
    dates = date_range.split('-')
    if len(dates) != 2:
        return abort(404)
    try:
        int(max_points_in_chart)
    except ValueError:
        return abort(404)
    ...
    if int(max_points_in_chart) > 15:
        max_points_in_chart = 15
    
    # For Graph and Bar-Chart, set average days per point
    total_days = number_of_logged_days(from_date, to_date)
    n_days_avg_per_point = round(total_days / int(max_points_in_chart))
    ...
    # Set at most 15 bars
    if int(max_points_in_chart) > MAX_POINTS_IN_BAR_CHART:
        max_points_in_chart = MAX_POINTS_IN_BAR_CHART
    ...
    return render_template('index.html', html_graph=graph, html_bar=bar, html_pie=pie)
